...
public interface ConcurrentCacheFactory {
    ...
    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent() {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), concurrencyStrategy(context));
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(ConcurrencyStrategy concurrencyStrategy) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), concurrencyStrategy);
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(long maxAttempts) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), maxAttempts, concurrencyStrategy(context));
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(long maxAttempts, ConcurrencyStrategy concurrencyStrategy) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), maxAttempts, concurrencyStrategy);
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(long maxAttempts, Duration minBackoff) {
    ...
    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(long maxAttempts, Duration minBackoff, ConcurrencyStrategy concurrencyStrategy) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), maxAttempts, minBackoff, concurrencyStrategy);
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetrySpec retrySpec) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), retrySpec, concurrencyStrategy(context));
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetrySpec retrySpec, ConcurrencyStrategy concurrencyStrategy) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), retrySpec, concurrencyStrategy);
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetryBackoffSpec retrySpec) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), retrySpec, concurrencyStrategy(context));
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetryBackoffSpec retrySpec, Scheduler retryScheduler) {
    ...
    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetryBackoffSpec retrySpec, ConcurrencyStrategy concurrencyStrategy) {
        return cacheFactory -> context -> concurrentCache(cacheFactory.create(context), retrySpec, concurrencyStrategy);
    }

    static <ID, EID, R, RRC> CacheTransformer<ID, EID, R, RRC> concurrent(RetryBackoffSpec retrySpec, ConcurrencyStrategy concurrencyStrategy, Scheduler retryScheduler) {
