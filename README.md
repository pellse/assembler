...
Publisher<BillingInfo> billingInfo(List<Long> customerIds); // Connects to a relational database with R2DBC
Publisher<OrderItem> allOrders(List<Long> customerIds); // Connects to MongoDB with Reactive Streams Driver
```
